...
function ProductsClient({ products }) {
  ...
  const filteredProducts = products
    .filter(product => product.category === "Boot")
    .filter(product =>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6 mt-10'>
        ...
        {filteredProducts.map(product => (
          ...
          <div className='flex items-center justify-between'>
            ...
            className='mt-4 bg-red-100 txt-red-700 rounded-xl py-2 px-4 text-center text-sm font-semibold'
            >
              Detayları gör
            </Link>
          </div>
